...
6. **Reviewer text means revise**: reviewer reply text 非空，则基于 `previous_draft + reviewer_reply_text` 生成下一版。
7. **Model output is content only**: LLM/RAG output 不能决定 routing、recipient、retry、mailbox state、reject 或 message movement。
...
v1 path：normalize input、route、optional retrieve；RAG 有 answer 则用 answer，否则 call `prompt_ali_p1_system.txt`。

v2+ path：必须有 `previous_draft`；用 `prompt_ali_p2_revision.txt` 和 `previous_draft + reviewer_reply_text` 生成 complete revised draft。不得 reroute、retrieve 或 fallback to rewrite semantics。
...
给 draft 加 ALI review header/footer，并分配 `[ALI:vN]` subject version。Packaging 不得改变 draft semantics 或 delivery policy。
