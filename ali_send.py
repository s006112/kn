...
# ------------------------------------------------------------
# Safety guard: ONLY reply to forward sender (engineer)
# ------------------------------------------------------------
...
def _require_reply_to_forward_sender(original_from: str, to_addr: str) -> None:
    ...
    Enforces that ALI can ONLY reply to the engineer who forwarded the email.
...
def _build_to_address(from_header: str) -> str:
    ...
    In forward-to-ALI workflow, original.from_addr MUST be the engineer,
    not the customer.
    """
...
def _build_message(
    ...
    # IMPORTANT:
    # This resolves to the forward sender (engineer), NOT the customer.
...
# ------------------------------------------------------------
# Public API: send_reply (ENGINEER-ONLY)
# ------------------------------------------------------------
...
def send_reply(
    ...
    *,
    ...
    """
    Send INTERNAL review email to engineer only.

    Parameters:
    - original:
        The forwarded EmailMessage.
        original.from_addr MUST be the engineer who forwarded the email.
    - reply_body:
