...
description: 以最小正确改动对既有 Python 代码进行修复/微重构，要求完全保留现有行为、删除冗余优先、仅在明确需求下改动，默认直接drop-in 替换代码。触发词包含：drop-in code、minimal necessary changes、preserve existing functionality、remove redundant duplicate code、no clever、boring.
---
...
## Output format
Default:
- A short “Changed files list” and “Behavior notes” (no long prose).
