...
- 持有当前 saved state
- 通过固定 polling loop 读取 live input
- 在唯一 accepted boundary 内做决策
...
## 2. System Ownership and Boundaries
...
用于 bootstrap 时从 live orders 直接识别合法 pair：
...
- bootstrap 不接受 `BUY_ONLY`
- bootstrap 不接受 `SELL_ONLY`
...
- engine 负责决定 bootstrap 是否接受该识别结果
...
## 10. Fill-Driven Rebuild Contract
...
- 必须使用刚成交那一侧的已知 canonical price 作为 rebuild anchor
- 不允许改用 mid
...
### Mid Unavailable Rule

若 `BTC mid` 不可用、无效或读取失败：

- 不得触发 anchor break
- 本轮不得改用推测值或历史值
- decision 仍只能基于当前有效输入继续判定

## 13. Main Loop Contract
...
order normalization 约束：

- 非法 side 必须视为非法输入
- 非法 side 不得进入 strategy decision
...
- 若 cleanup 输入不满足 cancel 所需字段，则视为 cleanup failure
...
## 16. Price Comparison Contract

engine / strategy / execution 内部关键价格判定必须基于离散化比较，而不是直接依赖 float 精确相等。
...
## 18. Exception Handling Contract
