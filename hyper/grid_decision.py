...
def decide_cycle_action(live_snapshot, saved_state):
    ...
    """
    orders = live_snapshot["orders"]
    live_state = live_snapshot["live_state"]
    current_btc_mid = live_snapshot["btc_mid"]
    saved_mode = saved_state["mode"]

    if saved_mode == PAIR_MODE:    # 填充检测：任一侧被成交则触发 rebuild
        if live_state["mode"] == SELL_ONLY_MODE:
            log_msg(f"🔥 BUY filled - {format_price(saved_state['buy_price'])}")
            return "rebuild", saved_state["buy_price"]
        if live_state["mode"] == BUY_ONLY_MODE:
            log_msg(f"✅ SELL filled - {format_price(saved_state['sell_price'])}")
            return "rebuild", saved_state["sell_price"]

        if (
            live_state["mode"] == PAIR_MODE
            and live_state["buy_price"] == saved_state["buy_price"]
            and live_state["sell_price"] == saved_state["sell_price"]
        ):
        ...
        if live_state["mode"] == BUY_ONLY_MODE:
            if REANCHOR_BREAK and current_btc_mid and current_btc_mid > 0:  # 重锚点检测 (Anchor Break)
                ...
                if price_distance_at_least(current_btc_mid, live_state["buy_price"], distance):
